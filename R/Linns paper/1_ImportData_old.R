...
temp16 <- read_delim("Data_plant_pollinator_Finse_2016_2017/2016/weather_2016.csv", locale = locale(decimal_mark = ",")) |> 
  select(date = `Tid(norsk normaltid)`, temperature = `Middeltemperatur (døgn)`, precipitation = `Nedbør (døgn)`) |> 
  mutate(date = dmy(date))
...
# get period for pollination for each plot
Period <- Biomass %>% 
...
WeatherAndBiomass <- bind_rows(meta16, meta17) |> 
  group_by(Year, BlockID, Plant) %>%
  summarise(CumTemp = sum(tempAboveZeroAdi, na.rm = TRUE)) |> 
...
WeatherAndBiomass <- WeatherAndBiomass %>% 
  bind_cols(d1, d2) %>% 
  rename(CumTemp.cen = V1...28, MeanFlower.cen = V1...29)
